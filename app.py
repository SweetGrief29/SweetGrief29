...
def api_manual_trade(body: dict = Body(...)):
    ...
    try:
        ...
        amount_token = body.get("amountToken")

        if amount_human is None and amount_token is not None:
            try:
                amount_human = float(amount_token)
        ...
        if amount_human is not None and amount_human <= 0:
            return JSONResponse({"error": "amountToken must be > 0"}, status_code=400)

        if amount_human is None:
            amt = float(amount_usd or 0)
            if amt <= 0:
                return JSONResponse({"error": "amountUsd must be > 0 or provide amountToken"}, status_code=400)
            try:
